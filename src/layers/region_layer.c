...
layer make_region_layer(int batch, int w, int h, int n, int classes, int coords,const char* mode)
{
    layer l = {0};
    l.type = REGION;
    ...
    l.forward_cudnn = forward_region_layer_gpu;
    l.backward_cudnn = backward_region_layer_gpu;
...
void resize_region_layer_gpu(layer *l, int w, int h)
{
...
void delta_region_class(float *output, float *delta, int index, int class, int classes, float scale, int stride, float *avg_cat, int tag)
{
    int i, n;
    ...
    for (b = 0; b < l.batch; ++b) {
        ...
        for(t = 0; t < 30; ++t){
            ...
            if(l.coords > 4){
                ...
                delta_region_mask(net.truth + t*(l.coords + 1) + b*l.truths + 5, l.output, l.coords - 4, mask_index,
																																								 l.delta, l.w*l.h, l.mask_scale);
            }
            if(iou > .5) recall += 1;
            avg_iou += iou;
            ...
            delta_region_class(l.output, l.delta, class_index, class, l.classes, l.class_scale, l.w*l.h, &avg_cat, !l.softmax);
            ++count;
            ++class_count;
        }
    }
...
void forward_region_layer_gpu(const layer l, network net)
{
    ...
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
            ...
            index = entry_index(l, b, n*l.w*l.h, l.coords + 1);
            if(!l.softmax ) activate_array_gpu(l.output_gpu + index, l.classes*l.w*l.h, LOGISTIC);
        }
    }
    if (l.softmax) {
        int index = entry_index(l, 0, 0, l.coords + !l.background);
